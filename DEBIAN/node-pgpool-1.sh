...
/usr/bin/ssh -T -l postgres $new_master "/usr/lib/postgresql/9.5/bin/repmgr -f /var/lib/pgsql/repmgr/repmgr.conf standby promote 2>/dev/null 1>/dev/null <&-"
 
exit 0;
) 2>&1 | tee -a /tmp/failover_stream.sh.log
EOF
chmod 755 /etc/pgpool2/failover_stream.sh
echo "вносим измененния в коннекты к pgpool2"
