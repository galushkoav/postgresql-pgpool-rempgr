...
TMPFILE='/etc/postgresql/ipinfo-$DATE_EXEC.txt'
        ...
        TEXT="$HOSTNAME следует за новым Мастером после файловера в кластере $CLUSTERNAME. ТЕКУЩЕЕ СОСТОЯНИЕ КЛАСТЕРА: Новый мастер `repmgr -f /etc/repmgr.conf cluster show | grep master | awk ' {print $4} '`  отказавшая нода `repmgr -f /etc/repmgr.conf cluster show | grep FAILED | awk ' {print $3} '`"
